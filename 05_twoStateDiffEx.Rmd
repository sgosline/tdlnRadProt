...
### Plot changing proteins in tumors
...
ggplot()+
  ...
  ggtitle('Patients by expression of differentially expressed proteins')+
  scale_color_manual(values=pal)

```

## Normal protein expression changes

Do the normal proteins also give indication of changes in LN status? 

```{r plot nomrmals, warning=FALSE}
#create data matrix of samples and collect patients
protMat<-normProts%>%
...
oneVsZero%>%subset(P.Value<0.005)%>%
  rename(Gene='featureID')%>%left_join(normProts)%>%
  left_join(red.dat)%>%
  ggplot(aes(x=Gene,y=inferredNormAbund))+geom_boxplot(aes(fill=lnSpread))+scale_x_discrete(guide = guide_axis(angle = 90))+ggtitle('Normal proteins somewhat differentially expressed at p<0.005')+scale_fill_manual(values=pal)

```

### Normal biological processes
What processes are enriched?

```{r normal biological pathways,warning=FALSE}
library(leapr)
...
```{r, plot nmormal samples,warning=FALSE}
zprots=subset(oneVsZero,P.Value<0.005)%>%select(featureID)
